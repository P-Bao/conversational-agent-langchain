...
class Config(BaseSettings):
    ...
    # fusion_algorithm: "rrf" (Reciprocal Rank Fusion, default) or "dbsf" (Distribution-Based Score Fusion).
    # Applied to the merge of dense (Gemini embedding) + sparse (BM25) search results.
    # NOTE: qdrant-client SDK only exposes the fusion algorithm choice; the smoothing constants
    # (rrf_k=60, dbsf_window=1000) are server-side defaults and are not tunable through this config.
    fusion_algorithm: str = Field(
        default="rrf",
        validation_alias=AliasChoices("fusion_algorithm", "FUSION_ALGORITHM", "hybrid_fusion"),
    )
    # Reserved for future use / documentation only (qdrant-client does not expose these today).
    # rrf_k: smoothing constant when fusion_algorithm == "rrf"; Qdrant server default is 60.
    rrf_k: int = Field(default=60, validation_alias=AliasChoices("rrf_k", "RRF_K"))
    # dbsf_window: normalization window when fusion_algorithm == "dbsf"; Qdrant server default is 1000.
