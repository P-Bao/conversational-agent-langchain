# Thuật Ngữ — Glossary (v8.1.0)

## A — M

| Thuật ngữ | Giải thích | Liên quan |
|---|---|---|
| **BGE-m3** | Multi-lingual embedding model của BAAI. Repo này **không chạy local** — gọi model qua HTTP endpoint ngoài (`EMBEDDING_BASE_URL`, GPU server). Trả cả dense (1024-dim) lẫn sparse vecs (hybrid search). | [ARCHITECTURE.md](ARCHITECTURE.md), [CONFIGURATION.md](CONFIGURATION.md), `src/agent/utils/embeddings.py` |
| **BGE Reranker** | Cross-encoder model `BAAI/bge-reranker-v2-m3`. Default gọi qua HTTP endpoint ngoài (`RERANK_BASE_URL`) khi `RERANK_PROVIDER=remote`; có thể chạy local với `RERANK_PROVIDER=bge`. So sánh query vs từng document pairs → ra relevance score. | [ARCHITECTURE.md](ARCHITECTURE.md), `src/agent/utils/reranker.py` |
...
| **Sparse Embedding** | Vector thưa (lexical/term weights) do BGE-m3 sinh cùng dense. Dùng cho hybrid search fusion (nâng recall với từ khoá). | [ARCHITECTURE.md](ARCHITECTURE.md), [CONFIGURATION.md](CONFIGURATION.md) |
| **FlagEmbedding** | Thư viện chính thức của BAAI cho BGE models. Không bắt buộc từ v7.1 (model chạy trên server HTTP ngoài) — chỉ cần nếu `RERANK_PROVIDER=bge` (local fallback). | `src/agent/utils/reranker.py` |
