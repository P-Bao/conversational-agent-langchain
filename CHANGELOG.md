...
- **Reranker mặc định chuyển sang remote HTTP**: `RERANK_PROVIDER` default đổi `bge` → **`remote`** (rerank server, vd `http://127.0.0.1:8010`). Contract hỗ trợ:
  - Request: `POST /rerank` với `{"query", "documents", "top_k"}`.
  - Response:
    - Contract đầy đủ: `{"scores": [float, ...], "ranked_indices": [int, ...]}`.
    - Contract chỉ trả scores: `{"scores": [float, ...]}` (client tự sort giảm dần và áp dụng `top_k`).
    - Fallback contract cũ: `{"results": [{index, score}]}` (Colab).
